      ...
      body: ListView(
        ...
        children: [
          ...
          _card(
            ...
            children: [
              _btn('login(@Body)', () => _run('login', () async {
                await _demoApi.login({'phone': '13800138000'});
                _setResult('login 请求已发送');
              })),
              _btn('getUserInfo()', () => _run('getUserInfo', () async {
                final r = await _demoApi.getUserInfo();
                _setResult(r != null ? 'getUserInfo: ${r.mobile ?? r.userId}' : 'null');
              })),
              _btn('saveArchives(@Body)', () => _run('saveArchives', () async {
          ...
          _card(
            ...
            children: [
              _btn('getList(page, size)', () => _run('getList', () async {
                final r = await _userApi.getList(1, 10);
                _setResult(r != null ? 'User: ${r.name ?? r.id}' : 'null');
              })),
              _btn('getById(id)', () => _run('getById', () async {
                final r = await _userApi.getById('123');
                _setResult(r != null ? 'User: ${r.name}' : 'null');
              })),
              _btn('getWithAuth(token)', () => _run('getWithAuth', () async {
                final r = await _userApi.getWithAuth('Bearer x');
                _setResult(r != null ? 'ok' : 'null');
              })),
              _btn('getByQuery(Map)', () => _run('getByQuery', () async {
                final r = await _userApi.getByQuery({'a': '1', 'b': '2'});
                _setResult(r != null ? 'ok' : 'null');
              })),
            ],
          ),
          _card(
            ...
            children: [
              _btn('create(@Body)', () => _run('create', () async {
                final r = await _articleApi.create({'title': 't', 'content': 'c'});
                _setResult(r != null ? 'Article: ${r.title}' : 'null');
              })),
              _btn('update(@Body)', () => _run('update', () async {
                final r = await _articleApi.update({'id': '1', 'title': 't2'});
                _setResult(r != null ? 'ok' : 'null');
              })),
              _btn('delete(id)', () => _run('delete', () async {
          ...
          _card(
            ...
            children: [
              _btn('getNested()', () => _run('getNested', () async {
